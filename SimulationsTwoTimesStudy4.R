...
setwd("C:\\Users\\kossi\\Dropbox\\RS_hormonotherapie\\Result_TwoTimes\\Study 4")
...
run_scenario = function(n, scenario) {
  ...
  for(i in 1:nrep){
    ...
    file.name = paste("TwoTimeStudy4_scenario_", scenario, "_", n, ".Rdata", sep="")
    save(results.logit,results.random,results.bayes,results.neural,
...
setwd("C:\\Users\\kossi\\Dropbox\\RS_hormonotherapie\\Result_TwoTimes\\Study 4")
load("TwoTimeStudy4_scenario_medium_300.Rdata")

### List of model results and their corresponding names
models = c("logit", "random", "bayes", "neural",  "SL")
...
#Pour le RQL
setwd("C:\\Users\\kossi\\Dropbox\\RS_hormonotherapie\\Result_TwoTimes\\Study 4")
load("RQL_TwoTimeStudy4_scenario_medium_300.Rdata")
...
#Remove extreme value # Medium Scenario

summary(results.RQL)
dev.new()
boxplot(results.RQL, main = "Distribution des estimés du RQL pour le scénario medium", 
        xlab = "Coefficients", ylab = "Valeurs estimées")
boite = boxplot(results.RQL, plot = FALSE)
limites = boite$ou

results.RQL.V2=results.RQL[,2]
index_min = which.min(results.RQL.V2)
selected_data = results.RQL.V2[-index_min]
#Biais
bias = mean(selected_data) - true.psi[2]
bias
#[1] -0.1196037

# Function to calculate standard deviation (sigma)
sig =sqrt(var(selected_data))
sig
#sig [1] 1.080564

# Function to calculate root mean squared error (rmse)
rmse =sqrt((mean(selected_data) - true.psi[2])^2 + var(selected_data))
rmse
#rmse 1.087163

#Ratio 1.087163/0.887009253006617

#Monte_Carlo Error
Monte_Carlo_SE_Bias= max(sig/sqrt(nrep))
Monte_Carlo_SE_Bias
Monte_Carlo_SE_sigma= max(sig/sqrt(2*(nrep-1)))
Monte_Carlo_SE_sigma